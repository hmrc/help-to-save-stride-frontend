...
class StrideController @Inject() (val authConnector: AuthConnector,
                                  val messagesApi:   MessagesApi)(implicit val frontendAppConfig: FrontendAppConfig)
  extends FrontendController with StrideAuth with I18nSupport {

  val getStartPage: Action[AnyContent] = authorisedFromStride{ implicit request ⇒
    Ok(views.html.start_page())
  }(routes.StrideController.getStartPage())
