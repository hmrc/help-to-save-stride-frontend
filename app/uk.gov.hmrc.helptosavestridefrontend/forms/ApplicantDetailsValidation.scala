...
class ApplicantDetailsValidationImpl @Inject() (configuration: FrontendAppConfig, clock: Clock)
  ...
  private def formatter[A](validate: String => Validated[NonEmptyList[String], A], mapping: Mapping[A]): Formatter[A] =
    new Formatter[A] {
    ...
    override def bind(key: String, data: Map[String, String]): Either[Seq[FormError], Option[String]] =
      key -> {
        data.get(key).map(_.trim).filter(_.nonEmpty) match {
          case Some(trimmed) =>
            validatedFromBoolean(trimmed)(_.length <= addressLineMaxTotalLength, ErrorMessages.tooLong).map(Some(_))
