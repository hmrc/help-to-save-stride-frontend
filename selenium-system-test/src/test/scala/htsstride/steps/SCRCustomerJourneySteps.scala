...
class SCRCustomerJourneySteps extends Steps with NINOGenerator {
  ...
  When("^the operator enters the first name as null$") {
    SCRCustomerEligiblePage.fillInSCRDetails(validCustomerDetails.copy(firstName = Some("")))
  }

  Then("^the enter a first name error message appears$") {
    Browser.checkCurrentPageIs(ErrorSCREligiblePage)
    ErrorSCREligiblePage.findErrorMessageList() shouldBe Some("Enter a first name")
    ()
  }
  ...
  Then("^the last name over the limit error message appears$") {
    Browser.checkCurrentPageIs(ErrorSCREligiblePage)
    ErrorSCREligiblePage.findErrorMessageList() shouldBe Some("Last name must be 300 characters or less")
    ()
  }
  ...
  Then("^the date of birth must be in the past error message appears$") {
    Browser.checkCurrentPageIs(ErrorSCREligiblePage)
    ErrorSCREligiblePage.findErrorMessageList() shouldBe Some("Date of birth must be in the past")
    ()
  }

  When("^the operator enters a month as 0 and the year as null$") {
    SCRCustomerEligiblePage.fillInSCRDetails(validCustomerDetails.copy(dobMonth = Some("0"), dobYear = Some("")))
  }

  Then("^the enter a date of birth error message appears$") {
    Browser.checkCurrentPageIs(ErrorSCREligiblePage)
    ErrorSCREligiblePage.findErrorMessageList() shouldBe Some("Enter a date of birth and include a day, month and year")
    ()
  }
  ...
  Then("^the enter a real date of birth error message appears$") {
    Browser.checkCurrentPageIs(ErrorSCREligiblePage)
    ErrorSCREligiblePage.findErrorMessageList() shouldBe Some("Enter a real date of birth")
    ()
  }
  ...
  Then("^the address line 1 error message appears$") {
    Browser.checkCurrentPageIs(ErrorSCREligiblePage)
    ErrorSCREligiblePage.findErrorMessageList() shouldBe Some("Address line 1 must be 35 characters or less")
    ()
  }

  When("^the operator enters null for the address line 2$") {
    SCRCustomerEligiblePage.fillInSCRDetails(validCustomerDetails.copy(addressLine2 = Some("")))
  }

  Then("^the enter an address line 2 error message appears$") {
    Browser.checkCurrentPageIs(ErrorSCREligiblePage)
    ErrorSCREligiblePage.findErrorMessageList() shouldBe Some("Enter an address line 2")
    ()
  }
  ...
  Then("^the enter a real postcode error message appears$") {
    Browser.checkCurrentPageIs(ErrorSCREligiblePage)
    ErrorSCREligiblePage.findErrorMessageList() shouldBe Some("Enter a real postcode")
    ()
  }
}
