...
SERVICES_STRING="$(sm -d | grep  HTS_STRIDE_SELENIUM  | cut -d '>' -f 2 | tr -d '[:space:]')"
...
if [ $status != 0 ]; then
  echo "*********** HTS_STRIDE_SELENIUM failed status check ***********"
else
  echo "*********** HTS_STRIDE_SELENIUM passed status check ***********"
fi
